...
let horizonServer = new StellarSdk.Server('https://horizon-testnet.stellar.org');
...
async function hashMemo(key = null, hash = null) {
    ...
    const account = await server.loadAccount(key || keypair.publicKey());
    const transaction = new StellarSdk.TransactionBuilder(account)
        .addOperation(StellarSdk.Operation.bumpSequence({
            bumpTo: account.sequenceNumber() + 1,
        }))
        .addMemo(new StellarSdk.Memo('hash', hash))
        .build();
